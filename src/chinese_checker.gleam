...
fn parse_dict_response(
  resp_result: Result(response.Response(String), fetch.FetchError),
) -> Result(Dict(String, DictEntry), String) {
  use response <- result.try(
    result.map_error(resp_result, fn(_) { "Failed to fetch dictionary" }),
  )
...
fn fetch_dictionary() -> Effect(Msg) {
  ...
  case request.to(url) {
    ...
    Ok(req) -> {
      ...
      |> promise.map(fn(resp) { dispatch(DictionaryLoaded(parse_dict_response(resp))) })
      Nil
    }
  }
}
...
fn do_segment_unknown(
  ...
  use <- bool.guard(pos >= len, list.reverse(acc))
...
fn build_cell_if_better(
  ...
  case best {
    Some(b) if new_score <= b.score -> best
    _ -> {
      let new_segs = case prev.unknown_start >= 0 {
        False -> prev.segments
        True -> {
          let unknown_segs =
            segment_unknown_text(substr(graphemes, prev.unknown_start, j), dict_keys)
          list.fold(unknown_segs, prev.segments, fn(acc, w) { [#(w, False), ..acc] })
        }
      }
      Some(DpCell(score: new_score, segments: [#(word, True), ..new_segs], unknown_start: -1))
    }
  }
}
...
fn try_known_word_at(
  ...
  use <- bool.guard(j >= end_i, best)
  let word = substr(graphemes, j, end_i)
  let new_best = case set.contains(known_words, word), dict.get(dp, j) {
    True, Ok(prev) -> build_cell_if_better(prev, word, j, graphemes, dict_keys, best)
    _, _ -> best
  }
  try_known_word_at(dp, j + 1, end_i, graphemes, known_words, dict_keys, new_best)
}
...
fn word_count_to_unknown(
  entry: #(String, #(Int, Bool)),
  the_dict: Dict(String, DictEntry),
) -> Result(UnknownWord, Nil) {
  case entry {
    #(word, #(count, False)) -> {
...
pub fn analyze(
  ...
  use <- bool.guard(!has_cjk(cleaned), None)
  ...
  use <- bool.guard(list.is_empty(valid_segments), None)
  ...
  let unknown_words =
    word_counts
    |> dict.to_list
    |> list.filter_map(fn(entry) { word_count_to_unknown(entry, the_dict) })
    |> list.sort(fn(a, b) {
